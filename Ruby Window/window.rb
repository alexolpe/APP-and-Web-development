...
class Finestra < Gtk::Window
	...
	def initialize
		...
		@display.show
		@display.style_context.add_provider(@css_provider,@style_provider)
    ...
    def create_table(resp)       
        ...
        if resp == "Error de Sintaxis"
            ...
            @grid.attach(label,0,4,4,1)
        elsif resp ==""
            label = Gtk::Label.new("Cerca Buida",{:use_underline =>false})
            label.set_name("labelvermell")
            label.style_context.add_provider(@css_provider,@style_provider)
            @grid.attach(label,0,4,4,1)    
        else
    ...
    def tag_on(body)
        #SI L'USUARI NO EXISTEIX PASSEM EL MISSATGE D'ERROR DEL SERVIDOR A FINESTRA 1 
            if body == "User not found: please login with your university card"
            
            #TORNEM A FINESTRA 1 I PASSEM PER PARAMETRE TRUE I BODY
            self.finestra1(true, body)
        else
            #SI L'USUARI EXISTEIX FEM UN JSON.PARSE I CANVIEM A LA PANTALLA 2
            var = JSON.parse(body)
            self.finestra2(var["name"])
        end
    end
    
    #FUNCIO FINESTRA1, LI PASSEM PER PARAMETRO UN BOOLEA QUE ENS PERMETRÀ SABER EN QUIN MOMENT ENTREM A FINESTRA1 PROCEDINT DE LA FUNCIO TAG_ON I UN STRING QUE CONTINDRA EL MISSATGE D'ERROR DEL SERVIDOR 
    def finestra1(bole_origin=false, str='')
